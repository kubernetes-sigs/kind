...
func (c *Context) Delete() error {
	...
	if err != nil {
		fmt.Printf("Tried to remove %s but recieved error: %s\n", c.KubeConfigPath(), err)
	}

	// check if $KUBECONFIG is set and let the user know to unset if so
	if os.Getenv("KUBECONFIG") == c.KubeConfigPath() {
		fmt.Printf("$KUBECONFIG is still set to use %s even though that file has been deleted, remember to unset it to get back to your original contexts\n", c.KubeConfigPath())
	}
