...
func TestDeriveInfo(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.TestName, func(t2 *testing.T) {
			...
			if err != nil {
				if !c.ExpectError {
					t2.Fatalf("unexpected error while Deriving infos: %v", err)
				}
				return
			}
			...
			if err == nil {
				if c.ExpectError {
					t2.Fatalf("unexpected lack or error while Deriving infos")
				}
			}

			// Fail if Nodes does not match
			checkReplicaList(t2, cfg.AllReplicas(), c.ExpectReplicas)

			// Fail if fields derived from Nodes does not match
			checkReplicaList(t2, cfg.ControlPlanes(), c.ExpectControlPlanes)
			checkNode(t2, cfg.BootStrapControlPlane(), c.ExpectBootStrapControlPlane)
			checkReplicaList(t2, cfg.SecondaryControlPlanes(), c.ExpectSecondaryControlPlanes)
			checkReplicaList(t2, cfg.Workers(), c.ExpectWorkers)
